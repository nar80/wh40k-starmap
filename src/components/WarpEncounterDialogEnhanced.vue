          ...
          <div class="row q-gutter-sm">
            <div class="col">
              <q-card class="bg-grey-9 text-white">
                <q-card-section>
                  <div class="text-caption">Hüllenschaden</div>
                  <div class="text-h6">{{ totalHullDamage }} HP</div>
                </q-card-section>
              </q-card>
            </div>
            <div class="col">
...
const generateCatastrophic = (num) => ({
  title: "Warp-Katastrophe",
  description: "Eine verheerende Warp-Anomalie trifft das Schiff mit voller Wucht.",
  effect: "Schwere Verluste sind unvermeidlich.",
  hullDamage: 5 + Math.floor(Math.random() * 5),
  crewLoss: 10 + Math.floor(Math.random() * 10),
  moraleLoss: 20 + Math.floor(Math.random() * 10),
  choices: ["Notfallprotokolle", "Schadensbegrenzung", "Beten"]
})

const generateSevere = (num) => ({
  title: "Schwere Warp-Störung",
  description: "Eine ernste Bedrohung manifestiert sich aus dem Warp.",
  effect: "Erhebliche Schäden und Verluste.",
  hullDamage: 3 + Math.floor(Math.random() * 4),
  crewLoss: 5 + Math.floor(Math.random() * 5),
  moraleLoss: 15 + Math.floor(Math.random() * 5),
  choices: ["Gegenwehr leisten", "Ausweichen", "Durchhalten"]
})

const generateModerate = (num) => ({
  title: "Warp-Turbulenz",
  description: "Moderate Turbulenzen erschüttern das Schiff.",
  effect: "Spürbare aber handhabbare Auswirkungen.",
  hullDamage: Math.random() > 0.5 ? 1 + Math.floor(Math.random() * 3) : 0,
  crewLoss: Math.random() > 0.5 ? 2 + Math.floor(Math.random() * 3) : 0,
  moraleLoss: 8 + Math.floor(Math.random() * 5),
  choices: ["Kurs halten", "Anpassen", "Abwarten"]
})

const generateMinor = (num) => ({
  title: "Kleine Störung",
  description: "Eine kleinere Anomalie stört die Reise.",
  effect: "Geringe Auswirkungen auf Schiff und Crew.",
  moraleLoss: 3 + Math.floor(Math.random() * 5),
  choices: ["Ignorieren", "Untersuchen", "Protokollieren"]
})

const generateNeutral = (num) => ({
  title: "Warp-Phänomen",
  description: "Ein ungewöhnliches aber harmloses Phänomen.",
  effect: "Keine direkten Auswirkungen.",
  choices: ["Beobachten", "Dokumentieren", "Weiterfliegen"]
})

const generateMild = (num) => ({
  title: "Glücklicher Umstand",
  description: "Ein leicht positives Ereignis während der Warp-Reise.",
  effect: "Kleine Verbesserung der Situation.",
  moraleGain: 2 + Math.floor(Math.random() * 4),
  choices: ["Nutzen", "Vorsichtig sein", "Dankbar sein"]
})

const generatePositive = (num) => ({
  title: "Fortunate Warp-Strömung",
  description: "Eine vorteilhafte Wendung im Warp.",
  effect: "Positive Auswirkungen für Schiff und Crew.",
  moraleGain: 5 + Math.floor(Math.random() * 5),
  crewGain: Math.random() > 0.7 ? 1 + Math.floor(Math.random() * 2) : 0,
  choices: ["Vorteil nutzen", "Vorsichtig bleiben", "Feiern"]
})

const generateBlessed = (num) => ({
  title: "Gesegnete Reise",
  description: "Der Imperator wacht über diese Reise.",
  effect: "Ausgezeichnete Bedingungen.",
  moraleGain: 10 + Math.floor(Math.random() * 10),
  choices: ["Dank aussprechen", "Segen teilen", "Demütig bleiben"]
})
...
const totalHullDamage = computed(() => {
  return rolls.value.reduce((sum, roll) => sum + (roll.encounter.hullDamage || 0), 0)
})
...
const applyEffectsAndClose = () => {
  ...
  $q.notify({
    ...
    caption: `HP: -${totalHullDamage.value}, Crew: ${totalCrewChange.value > 0 ? '+' : ''}${totalCrewChange.value}%, Moral: ${totalMoraleChange.value > 0 ? '+' : ''}${totalMoraleChange.value}`,
    position: 'top'
  })
